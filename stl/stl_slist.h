...
// Forward declarations of operator==, needed for friend declaration.
...
INLINE_LOOP const _Slist_node_base* 
...
INLINE_LOOP _Slist_node_base* __slist_reverse(_Slist_node_base* __node)
{
...
INLINE_LOOP size_t __slist_size(_Slist_node_base* __node)
{
  size_t __result = 0;
...
# if defined ( __STL_DEBUG )
  _Node* _Get_iterator() const { return (_Node*)_M_node; }  
  ...
  _Slist_iterator(const iterator& __x) : 
      _Slist_iterator_base(__x._Owner(), __x._M_node) {}
# else
...
# endif
  ...
  _Self& operator++()
  {
    __stl_verbose_assert(_M_node != 0, __STL_MSG_INVALID_ADVANCE); 
    _M_incr();
    return *this;
  }
  _Self operator++(int)
                             ...
                             _Alloc_traits<_Tp, _Alloc>::_S_instanceless>
{
  ...
  _Slist_base(const allocator_type& __a) : _Base(__a) { 
    __stl_debug_do(_M_iter_list._Safe_init(&_M_head));
    _M_head._M_next = 0; 
  }
  ~_Slist_base() { _M_erase_after(&_M_head, 0); }
#if defined( __STL_HAS_NAMESPACES )
  __STL_USING_BASE_MEMBER _Slist_alloc_base<_Tp, _Alloc,
                             _Alloc_traits<_Tp, _Alloc>::_S_instanceless>::_M_get_node;
  __STL_USING_BASE_MEMBER _Slist_alloc_base<_Tp, _Alloc,
                             _Alloc_traits<_Tp, _Alloc>::_S_instanceless>::_M_put_node;
  __STL_USING_BASE_MEMBER _Slist_alloc_base<_Tp, _Alloc,
                             _Alloc_traits<_Tp, _Alloc>::_S_instanceless>::_M_head;
...
protected:
#if defined( __STL_HAS_NAMESPACES )
  __STL_USING_BASE_MEMBER _Slist_base<_Tp,_Alloc>::_M_get_node;
  __STL_USING_BASE_MEMBER _Slist_base<_Tp,_Alloc>::_M_put_node;
  __STL_USING_BASE_MEMBER _Slist_base<_Tp,_Alloc>::_M_erase_after;
  __STL_USING_BASE_MEMBER _Slist_base<_Tp,_Alloc>::_M_head;
#endif /* __STL_USE_NAMESPACES */

public:
  explicit slist(const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM)) : _Slist_base<_Tp,_Alloc>(__a) {}

  slist(size_type __n, const value_type& __x,
        const __STL_ALLOC_PARAM& __a =  __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM)) : _Slist_base<_Tp,_Alloc>(__a)
...
#  define _Make_const_iterator(__l) const_iterator(&_M_iter_list, (__l))
  void _Invalidate_node(_Node* __it) {__invalidate_iterator(&_M_iter_list, __it, end()); }
  void _Invalidate_iterator(const iterator& __it) {__invalidate_iterator(&_M_iter_list, __it._M_node, __it); }
# else
#  define _Make_iterator(__l) iterator((__l))
#  define _Make_const_iterator(__l) const_iterator((__l))
# endif

  iterator begin() { return _Make_iterator((_Node*)_M_head._M_next); }
  const_iterator begin() const 
    { return _Make_const_iterator((_Node*)_M_head._M_next);}
...
public:
  ...
  iterator previous(const_iterator __pos) {
    return _Make_iterator((_Node*) __slist_previous(&_M_head, __pos._M_node));
  }
  const_iterator previous(const_iterator __pos) const {
    return _Make_const_iterator((_Node*) __slist_previous(&_M_head, __pos._M_node));
  }
...
public:
  ...
  iterator insert(iterator __pos, const value_type& __x) {
    ...
    return _Make_iterator(_M_insert_after(__slist_previous(&_M_head, __pos._M_node),
                    __x));
  }
...
public:
  ...
  iterator erase(iterator __pos) {
    __stl_debug_check(__check_if_owner(&_M_iter_list,__pos));
    return _Make_iterator((_Node*) _M_erase_after(__slist_previous(&_M_head, 
                                                    __pos._M_node)));
  }
...
public:
  ...
  void splice_after(iterator __pos, iterator __prev)
  {
    ...
    __stl_debug_do(++__prev;
		   __invalidate_iterator(__prev._Owner(), __prev._M_node, __prev));
  }
