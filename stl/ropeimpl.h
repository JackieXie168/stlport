...
# ifdef __STL_USE_NEW_STULE_HEADERS
#  include <cstdio>
# else
...
# if defined(__STL_USE_STD_ALLOCATORS)
	        _L_deallocate(__l, 1);
# else
	        rope<_CharT, _Alloc>::_L_deallocate(__l, 1);
# endif
	        break;
	    }
	case _S_concat:
...
# if defined(__STL_USE_STD_ALLOCATORS)
	        _C_deallocate(__c, 1);
# else
	        rope<_CharT, _Alloc>::_C_deallocate(__c, 1);
# endif
	        break;
	    }
	case _S_function:
...
# if defined(__STL_USE_STD_ALLOCATORS)
	        _F_deallocate(__f, 1);
# else
	        rope<_CharT, _Alloc>::_F_deallocate(__f, 1);
# endif
	        break;
	    }
	case _S_substringfn:
...
# else
	        rope<_CharT, _Alloc>::_S_deallocate(__ss, 1);
# endif
		break;
	    }
    }
...
template <class _CharT, class _Alloc>
__RopeRep__*
rope<_CharT,_Alloc>::_S_concat(__RopeRep__* __left, __RopeRep__* __right)
{
    if (0 == __left) {
	_S_ref(__right);
	return __right;
...
# define  __exponentiate_threshold size_t(32)
    ...
    typedef simple_alloc<_CharT, _Alloc> _DataAlloc;
