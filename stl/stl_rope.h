  ...
  public:
    ...
    _Rope_RopeConcatenation(_Rope_RopeRep<_CharT,_Alloc>* __l,
      ...
      :   _Rope_RopeRep<_CharT,_Alloc>(
          ...
          __l->_M_size + __r->_M_size, __a), _M_left(__l), _M_right(__r)
      {}
# ifndef __GC
...
/*   : public random_access_iterator<_CharT, ptrdiff_t>  */
{
  ...
  //  protected:
    ...
    static void _S_setbuf(_Self& __x);
                                        // Set buffer contents given
                                        // path cache.
    static void _S_setcache(_Self& __x);
                                        // Set buffer contents and
                                        // path cache.
    static void _S_setcache_for_incr(_Self& __x);
...
public:
  ...
  public:
    ...
    inline _Rope_iterator(rope<_CharT,_Alloc>& __r, size_t __pos);
    ~_Rope_iterator() {
...
public:
          ...
          __STL_USING_BASE_MEMBER _Rope_base<_CharT,_Alloc>::_M_tree_ptr;
#       endif
        typedef _Rope_RopeRep<_CharT,_Alloc> _RopeRep;
...
#           endif
            return __STL_PLACEMENT_NEW(__space) _RopeLeaf(__s, __size, __a);
        }
...
#           endif
            return __STL_PLACEMENT_NEW(__space) _RopeFunction(__f, __size, __d, __a);
        }
...
public: /* for operators */
        rope(_RopeRep* __t, const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
          : _Rope_base<_CharT,_Alloc>(__t,__a) { }
        private:
...
protected:
   ...
   public:
        ...
        rope(const _CharT* __s, const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(__STL_ROPE_FROM_UNOWNED_CHAR_PTR(__s, _S_char_ptr_len(__s),
                                                 __a),__a)
        { }

        rope(const _CharT* __s, size_t __len,
             const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(__STL_ROPE_FROM_UNOWNED_CHAR_PTR(__s, __len, __a), __a)
        { }
        ...
        rope(const _CharT *__s, const _CharT *__e,
             const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(__STL_ROPE_FROM_UNOWNED_CHAR_PTR(__s, __e - __s, __a), __a)
        { }

        rope(const const_iterator& __s, const const_iterator& __e,
             const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(_S_substring(__s._M_root, __s._M_current_pos,
        ...
        rope(_CharT __c, const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(__a)
        {
        ...
        rope(size_t __n, _CharT __c,     
	     const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM));
        rope(const __STL_ALLOC_PARAM& __a = __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(0, __a) {}
        ...
        rope(const _Self& __x, const __STL_ALLOC_PARAM& __a = 
	     __STL_ALLOC_INSTANCE(__STL_ALLOC_PARAM))
        : _Rope_base<_CharT,_Alloc>(__x._M_tree_ptr, __a)
        {
            _S_ref(_M_tree_ptr);
        }
...
#     else /* __STL_CLASS_PARTIAL_SPECIALIZATION */
# if defined (__STL_MSVC50_COMPATIBILITY)
	typedef __STD::reverse_iterator<iterator, value_type, reference,
...
const __size_type__ rope<_CharT, _Alloc>::npos =
			(__size_type__)(-1);
# else
const __size_type__ rope<char, alloc>::npos __STL_WEAK = (__size_type__)(-1);
#  if defined (__STL_WCHAR_T)
const __size_type__ rope<wchar_t, alloc>::npos __STL_WEAK = (__size_type__)(-1);
#  endif
# endif
...
# else
    rope<_CharT, _Alloc>::_Data_deallocate(__cstr, __size);
# endif
  }
}
...
template<class _CharT, class _Alloc>
void 
_Rope_RopeRep<_CharT, _Alloc>::_S_free_string( __GC_CONST _CharT* __s, size_t __len
#   ifdef __STL_USE_STD_ALLOCATORS
      , allocator_type __a
#   endif
)
...
#   ifdef __STL_USE_STD_ALLOCATORS
        __a.deallocate(
	    __s, _S_rounded_up_size(__len));
#   else
	rope<_CharT, _Alloc>::_Data_deallocate(
					       __s, _S_rounded_up_size(__len));
#   endif
}
