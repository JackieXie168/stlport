...
//#   ifdef _MSC_VER
//    friend inline bool operator != __STL_NULL_TMPL_ARGS (const __rope_const_iterator<charT,Alloc> & x,
//							 const __rope_const_iterator<charT,Alloc> & y);
//#   endif
    friend inline bool operator< __STL_NULL_TMPL_ARGS
	(const __rope_const_iterator<charT,Alloc> & x,
	 const __rope_const_iterator<charT,Alloc> & y);
    friend inline ptrdiff_t operator- __STL_NULL_TMPL_ARGS
	(const __rope_const_iterator<charT,Alloc> & x,
	 const __rope_const_iterator<charT,Alloc> & y);
};
...
class __rope_iterator : public __rope_iterator_base<charT,Alloc> {
  ...
  public:
	...
	(const __rope_iterator<charT,Alloc> & x,
	 const __rope_iterator<charT,Alloc> & y);
#if !defined( __MWERKS__ )  || __MWERKS__ >= 0x2000		// dwa 8/21/97  - "ambiguous access to overloaded function" bug.
    friend inline __rope_iterator<charT,Alloc> operator- __STL_NULL_TMPL_ARGS
	(const __rope_iterator<charT,Alloc> & x,
	 ptrdiff_t n);
#else
...
#     endif /* __STL_CLASS_PARTIAL_SPECIALIZATION */ 
	...
	friend inline rope<charT,Alloc> operator+ __STL_NULL_TMPL_ARGS (const rope<charT,Alloc> &left,
...
template<class charT, class Alloc>
struct __rope_concat_fn : public binary_function< rope<charT, Alloc>, 
